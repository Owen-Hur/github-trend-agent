...
```
[실행 트리거: GitHub Actions Cron (화·금 09:00 KST) / 수동 실행]
           │
           ▼
┌────────────────────────────────────────────────────────┐
...
cron `0 0 * * 2,5` (화·금 00:00 UTC = 09:00 KST).
한때 매일 실행으로 바꿨으나, 측정상 회당 별점대가 ⭐100~300으로 화·금(⭐392~493)보다 낮아 원래 설계로 되돌렸다.
분야별 트랙은 `TOPIC_WEEKDAYS = [4]` 에 따라 **금요일 실행분에만** 붙는다.
...
| Phase 7 | GH Actions cron 배포 | 개입 없이 화·금 자동 발송 | **완료 ✅ 실행 검증** |
