...
```
[실행 트리거: GitHub Actions Cron (화·금) / 수동 실행]
           │
           ▼
┌────────────────────────────────────────────────────────┐
...
cron은 `17 0 * * 2,5` (화·금 00:17 UTC = 09:17 KST). 정각은 Actions 스케줄러 혼잡 구간이라 분을 어긋나게 잡는다.
...
on:
  schedule:
    - cron: '17 0 * * 2,5'   # 화·금 00:17 UTC (09:17 KST)
  workflow_dispatch: {}
permissions:
...
| Phase 4 | Claude tool schema 4개 필드 | 4개 필드 모두 누락 없이 생성 확인 | 구현 완료 — **API 키 대기** |
| Phase 5 | Slack Webhook 연동 (Block Kit) | 채널에 서식 깨짐 없이 메시지 수신 | 구현 완료 — **웹훅 URL 대기** |
| Phase 6 | 품질 하한선 로직 | 후보 부족 시 건수 축소 발송 확인 | **완료 ✅ 단위 테스트** |
| Phase 7 | GH Actions cron 배포 | 개입 없이 화·금 자동 발송 | 워크플로 작성 완료 — **저장소 대기** |
