...
- 웹훅 URL당 초당 1회 전송 제한이 있으나, 하루 1회 실행이므로 무관하다.
...
주기별로 회당 뽑히는 상위 5건의 별점대를 실측하면:

| 주기 | 회당 상위 5건 별점대 | 주간 발송량 |
|---|---|---|
| **매일 (현재 설정)** | **⭐100 ~ 300** | **35건** |
| 화·금 | ⭐392 ~ 493 | 10건 |
| 주 1회 | ⭐332 ~ 1,127 | 5건 |

자주 보낼수록 회당 품질은 내려간다. 상위권을 먼저 소비하고 그 아래로 내려가기 때문이다.
**현재는 매일 발송으로 설정**돼 있다. 회당 품질을 올리고 싶으면 `PICK_COUNT`를 5→3으로 낮추면 상위권만 남는다.
...
언어를 바꾸려면 `BRIEFING_LANG` 환경변수를 쓴다. GitHub Actions에서는 저장소 **Settings → Secrets and variables → Variables**에 `BRIEFING_LANG`을 추가하면 된다.

```bash
BRIEFING_LANG=English python3 -m src.main --dry-run --no-llm
```
...
배지·HTML 제거, 품질 하한선, 상태 파일 영속화·파기·손상 복구, Block Kit 제한(50블록/3,000자), mrkdwn 이스케이프, 전달 대상 결정, 시크릿 사전 검증, 언어 전환, fallback 동작을 덮는다.
